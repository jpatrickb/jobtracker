...
- **Review is no longer something you can trigger yourself.** This agent runs with `task: deny` in its
  permission config, so it has no ability to dispatch another agent from this session. Your job ends with
  a draft plus an honest verdict on whether it's been independently reviewed yet — never with an
  unprompted claim that review happened.
...
**You cannot dispatch `resume-reviewer` yourself.** That used to happen automatically at this point; this
agent runs with `task: deny` — no ability to dispatch another agent from this session. Instead, your
final report must end with an unmissable line naming the folder and the required next step, so whoever
dispatched you (a human, or the orchestrating session) knows review is still outstanding:
