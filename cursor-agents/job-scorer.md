...
# readonly is deliberately false, not true: Cursor has no per-tool allowlist (unlike Claude Code's
# `tools: Read, Glob, Grep, WebFetch, Bash`), only this one coarse on/off switch, and this agent's
# defining behavior is persisting every score to the tracker via `jobtracker add`/`rescore`/`set-*`
# run through shell commands. `readonly: true` would block that and break the agent's actual job.
readonly: false
# model omitted (defaults to `inherit`): Claude Code pins this agent to the lightweight `haiku`
# model via `model: haiku`; Cursor's `model:` takes a Cursor/OpenAI/etc model id, which has no
# direct equivalent, so there is nothing faithful to port here. Set explicitly if you want a
# cheaper model for bulk dispatch.
---
...
<id>` records the new one without destroying the old). See the version rule in `AGENTS.md`.
