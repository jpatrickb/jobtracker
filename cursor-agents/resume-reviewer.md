...
# readonly: true is the right call here, unlike job-scorer: this agent's own "Feedback handling"
# section is explicit that it proposes durable updates (to EVIDENCE.md/BULLETS.md/REVIEW_RUBRIC.md)
# for a yes/no rather than writing them itself. The one caveat is step 7's `typst compile`, which
# writes a PDF as part of the formatting check -- if your Cursor version enforces `readonly` against
# that too, relax this to `false` for this agent specifically.
readonly: true
---
