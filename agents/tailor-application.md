...
- **Review is no longer something you can trigger yourself.** You cannot dispatch another subagent from
  inside an isolated agent context. Your job ends with a draft plus an honest verdict on whether it's
  been independently reviewed yet — never with an unprompted claim that review happened.
...
it can't from an isolated agent context. Instead, your final report must end with an unmissable line
naming the folder and the required next step, so whoever dispatched you (a human, or the orchestrating
Claude Code session) knows review is still outstanding:
