...
- **Review is no longer something you can trigger yourself.** This file's own `tools:` frontmatter
  (above) deliberately omits Pi's `subagent` dispatch tool, so you have no way to spawn another agent
  even if you tried — this is an enforced restriction, not just an instruction you're trusting yourself
  to follow. Your job ends with a draft plus an honest verdict on whether it's been independently
  reviewed yet — never with an unprompted claim that review happened.
...
- **A URL:** fetch it with the `bash` tool (Pi has no built-in URL-fetch tool), e.g. `curl -fsSL "<url>"`,
  strip HTML markup into readable text if the response isn't already plain text, and save the extracted
  text as `job-description.md`. If the site refuses non-browser requests, say so in your final report
  and ask for the posting text to be pasted instead rather than proceeding without it.
...
**You cannot dispatch `resume-reviewer` yourself.** That used to happen automatically at this point;
it can't from an isolated agent context, and this file's own `tools:` frontmatter enforces it by
omitting Pi's `subagent` dispatch tool. Instead, your final report must end with an unmissable line
naming the folder and the required next step, so whoever dispatched you (a human, or the orchestrating
Pi session) knows review is still outstanding:
