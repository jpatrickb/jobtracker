...
If given a URL instead of pasted text, retrieve the posting first with the `bash` tool, since Pi has no
built-in URL-fetch tool: `curl -fsSL "<url>"` (add `-A "Mozilla/5.0"` if the request is refused). The
response is usually raw HTML rather than plain text; if so, strip it into readable text before treating
it as the posting, e.g. `curl -fsSL "<url>" | python3 -c "import re,sys,html; print(html.unescape(re.sub(r'<[^>]+>', ' ', sys.stdin.read())))"`,
then read the result yourself and ignore obvious nav/script/footer noise. If the site blocks
non-browser requests entirely (some ATSes do), say so plainly and ask the user to paste the posting
text instead rather than guessing at content you couldn't retrieve. If given a file path, read it
directly. If asked to process the inbox (or invoked with nothing specific to score), first run `jobtracker inbox-dupes` from the repo root: for any "Already scored" hit, delete that inbox file without scoring it (it's already in `listings/`); for any "Duplicate posting"/"Similar titles" pair, skip all but one of the group. Then `ls inbox/` and score every remaining file in turn, one full pass of this procedure each, in the order listed.
...
2. **Save the raw listing text verbatim** (the pasted text, or the fetched/Read page content,
